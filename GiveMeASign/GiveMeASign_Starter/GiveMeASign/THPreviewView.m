...
+ (Class)layerClass {
    // overwrite layerClass method
    // added AVCaptureVideoPrevieLayer object as a return value
    return [AVCaptureVideoPreviewLayer class];
}

- (AVCaptureSession*)session {
    // overwrite session method
    // return session object
    return [(AVCaptureVideoPreviewLayer*)self.layer session];
}

- (void)setSession:(AVCaptureSession *)session {
    // access layer property in setSession
    // AVCaptureVideoPreviewLayer intance
    // Add captured data into preview layer
    [(AVCaptureVideoPreviewLayer*)self.layer setSession:session];
}


// UI gestures: tap, double-tap, focus and expose
- (void)setupView {
...
// private method to support different touching
